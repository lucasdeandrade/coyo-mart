      ...
      <div class="modal-content p-4 dialog">
        ...
        <div class="modal-body pt-2">
          <div class="row justify-content-between pt-3 px-3">
            ...
            <button
              type="button"
              class="col-2 mx-3 btn btn-outline-danger btn-sm rounded"
  ...
  <div class="col-11 shadow">
    <div class="row justify-content-between pt-3 px-3">
      ...
      <div class="col-4 align-self-end input">
        <div class="row mt-2">
          <button
            class="col-3 btn btn-outline-primary coyoButton"
            @click="this.category = {}"
        ...
        <tbody>
          ...
          >
            ...
            <td>
              <p>
                {{
                  `${new Date(category.created_at).getDate()}/${
                    new Date(category.created_at).getMonth() + 1
                  }/${new Date(category.created_at).getFullYear()}`
                }}
              </p>
            </td>
          </tr>
  ...
  methods: {
    ...
    async submit() {
      if (this.category.id) {
        await axios
          .put(
            `http://127.0.0.1:3000/categories/${this.category.id}`,
            this.category
          )
          .then();
        location.reload();
    ...
    async deleteCategory() {
      var productsWithThisCategory = this.$store.state.products.filter(
        (product) => {
      ...
      if (productsWithThisCategory == 0) {
        ...
        alert(
          `Não é possivel deletar essa categoria, existe ${productsWithThisCategory} produto(s) vinculados a ela`
        );
      }
    },
  },
